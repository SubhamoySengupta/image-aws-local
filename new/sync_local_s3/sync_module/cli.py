...
def sync_delete_from_s3(path, local_path, dest):
	out_path = path + '/out/deleted.txt'
	aws_cmd = 'aws s3 sync ' + local_path + ' ' + dest + ' --delete >' + out_path
	process = Popen([aws_cmd], shell=True, stdin=PIPE, stderr=PIPE)
	print process.communicate()


def sync_delete_from_local(path, local_path, dest):
	out_path = path + '/out/deleted-1.txt'
	aws_cmd = 'aws s3 sync --delete ' + dest + ' ' + local_path + ' >' + out_path
	process = Popen([aws_cmd], shell=True, stdin=PIPE, stderr=PIPE)
	print process.communicate()
