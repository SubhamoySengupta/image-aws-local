...
def task(arg):
	if len(arg) > 1:
		options = dict(dryrun=dryrun, sync=sync, help=help, configure=config, sync_delete=sync_delete)
		options[arg[1]]()
	else:
...
def sync_delete():
	print 'Deleting'
	source, dest = sync_module.paths.get_path(path)
	# sync_module.cli.sync_delete_from_local(path, source, dest)
	sync_module.cli.sync_delete_from_s3(path, source, dest)
